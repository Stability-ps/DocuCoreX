...
export class MockConversionProvider implements ConversionProvider {
  ...
  async run(
    ...
    const extension = options.toFormat === "excel" ? "xlsx" : options.toFormat === "word" ? "docx" : options.toFormat;
    const fileName = `${document.name.replace(/\.[^.]+$/, "")}.${extension}`;

    return {
      id: `conversion_${document.id}`,
      status: "ready",
      downloadPath: `${document.workspaceId}/conversions/${document.id}/${fileName}`,
      fileName,
      contentType: getConversionContentType(extension),
      content: [
        "DocuCoreX converted output",
        `Document: ${document.name}`,
        `Target format: ${options.toFormat}`,
        "Provider: mock conversion fallback",
      ].join("\n"),
      message: `Conversion to ${options.toFormat} completed`,
    };
  }
}
...
function getConversionContentType(extension: string) {
