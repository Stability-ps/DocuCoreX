...
export class MockConversionProvider implements ConversionProvider {
  ...
  async run(
    ...
    const lines = [
      ...
      `Original filename: ${document.name}`,
      `Original MIME type: ${document.mimeType}`,
      `Original size: ${document.sizeBytes} bytes`,
      `Detected type: ${document.detectedType}`,
      "Generated by DocuCoreX local conversion provider.",
      "For full visual fidelity, configure a dedicated conversion provider.",
    ];
    const generated =
      options.toFormat === "pdf"
        ? createPdfFile(document.name, lines)
        : options.toFormat === "word"
          ? createDocxFile(document.name, lines)
          : options.toFormat === "excel"
            ? createXlsxFile(document.name, [
                ["Field", "Value"],
                ["Document ID", document.id],
                ["Original filename", document.name],
                ["MIME type", document.mimeType],
                ["Size bytes", String(document.sizeBytes)],
                ["Detected type", document.detectedType],
              ])
            : options.toFormat === "zip"
              ? {
                  fileName: `${document.name.replace(/\.[^.]+$/, "").replace(/[^a-zA-Z0-9._-]/g, "_") || "document"}.zip`,
                  contentType: "application/zip",
                  content: createZip([
                    {
                      name: "docucorex-processing-summary.txt",
                      content: new TextEncoder().encode(lines.join("\n")),
                    },
                  ]),
                }
              : createTextFile(document.name, lines);
