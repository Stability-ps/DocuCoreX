...
export function IntegrationsConsole() {
  ...
  return (
    <div className="space-y-4">
      {status ? <p className="rounded-2xl border border-royal-100 bg-royal-50 px-4 py-3 text-sm font-semibold text-royal-800">{status}</p> : null}
      {integrationConnectors.map((connector) => {
        ...
        return (
          <section key={connector.name} className="rounded-xl border border-slate-200 bg-white shadow-sm">
            <div className="flex flex-col gap-4 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="font-semibold text-navy-950">{connector.name}</h2>
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className={`inline-flex items-center justify-center gap-2 rounded-2xl px-4 py-3 text-sm font-semibold ${
                    isConnected ? "bg-emerald-50 text-emerald-700" : "bg-royal-600 text-white shadow-sm"
                  }`}
                >
                ...
                {isConnected ? (
                  <button
                    ...
                    className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-600 shadow-sm disabled:cursor-wait disabled:opacity-60"
                  >
                    Disconnect
                  </button>
                ) : null}
            ...
            {setupId === id ? (
              <div className="border-t border-slate-100 p-5">
                <p className="mb-4 rounded-2xl border border-royal-100 bg-royal-50 p-4 text-sm font-semibold text-royal-900">
                ...
                <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
                  ...
                  <button
                    ...
                    disabled={pendingId === id}
                    className="rounded-2xl bg-royal-600 px-4 py-3 text-sm font-semibold text-white shadow-sm disabled:cursor-wait disabled:bg-slate-300"
                  >
